...
var CameraControls = function ( object, domElement ) {
	...
	if ( domElement === document ) console.error( 'THREE.CameraControls: "document" should not be used as the target "domElement". Please use "g_renderer.domElement" instead.' );
	...
	this.update = function () {
		...
		// so g_camera.up is the orbit axis
		...
		return function update() {
			...
			if ( scope.trackball ) {
				...
				// rotate offset back to "g_camera-up-vector-is-up" space
			...
			// update condition is:
			// min(g_camera displacement, g_camera rotation in radians)^2 > EPS
	...
	var pan = function () {
		...
		return function pan( deltaX, deltaY ) {
			...
			if ( scope.object.isPerspectiveCamera ) {
				...
				// g_camera neither orthographic nor perspective
				console.warn( 'WARNING: CameraControls.js encountered an unknown g_camera type - pan disabled.' );
	...
	function dollyIn( dollyScale ) {
		...
		if ( scope.object.isPerspectiveCamera ) {
			...
			console.warn( 'WARNING: CameraControls.js encountered an unknown g_camera type - dolly/zoom disabled.' );
...
// OrbitControls maintains the "up" direction, g_camera.up (+Y by default).
//
...
// MapControls maintains the "up" direction, g_camera.up (+Y by default)
//
...
// TrackballControls allows the g_camera to rotate over the polls and does not maintain g_camera.up
//
//    Orbit - left mouse / touch: one-finger move
//    Zoom - middle mouse, or mousewheel / touch: two-finger spread or squish
//    Pan - right mouse, or left mouse + ctrl/meta/shiftKey, or arrow keys / touch: two-finger move
